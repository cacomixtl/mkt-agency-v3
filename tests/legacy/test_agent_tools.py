...
async def test_check_pending_campaign_none(mock_db_unavailable):
    ...
    from app.services.agent.service import _campaign_drafts, check_pending_campaign
...
async def test_select_caption_a(mock_db_unavailable):
    ...
    from app.services.agent.service import _campaign_drafts, select_caption
...
async def test_schedule_campaign(mock_db_unavailable):
    ...
    from app.services.agent.service import _campaign_drafts, schedule_campaign
    ...
    assert (
        _campaign_drafts["+5215551234567"]["strategy"]["schedule"] == "2026-03-15 10:00"
    )
...
async def test_publish_campaign_hitl(mock_db_unavailable):
    ...
    from app.services.agent.service import _campaign_drafts, publish_campaign
...
async def test_reject_no_draft(mock_db_unavailable):
    ...
    from app.services.agent.service import _campaign_drafts, reject_and_regenerate
...
def test_output_guardrail_blocked():
    ...
    from app.services.agent.service import FALLBACK_MESSAGE, _check_output_guardrail
...
def test_extract_action_schedule():
    ...
    result = _extract_action(
        ["CAMPAIGN_SCHEDULED|datetime:2026-03-15 10:00|SCHEDULE_APPROVED"], ""
    )
